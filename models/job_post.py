...
class JobPostModel(db.Model):
    ...
    job_post_id = db.Column(db.Integer, primary_key=True, autoincrement=True, nullable=False)
    ...
    # foreign key
    business_profile_id = db.Column(db.Integer, db.ForeignKey("business_profile.business_profile_id"), nullable=False)
    
    # relationship
    business_profile = db.relationship("BusinessProfileModel", back_populates="job_post")
    application = db.relationship("ApplicationModel", back_populates="job_post", lazy="dynamic", cascade="all, delete")
    post_premium = db.relationship("PostPremiumModel", back_populates="job_post", lazy="dynamic", cascade="all, delete")
    bookmark = db.relationship("BookmarkModel", back_populates="job_post", lazy="dynamic", cascade="all, delete")
