...
class AccountModel(db.Model):
    ...
    account_id = db.Column(db.Integer, primary_key=True, autoincrement=True, nullable=False)
    ...
    # relationship
    user_profile = db.relationship("UserProfileModel", back_populates="account", lazy="dynamic", cascade="all, delete")
    business_profile = db.relationship("BusinessProfileModel", back_populates="account", lazy="dynamic", cascade="all, delete")
    bookmark = db.relationship("BookmarkModel", back_populates="account", lazy="dynamic", cascade="all, delete")
    ...
    def reset_password(self, reset_password_token):
        ...
        msg.body = 'Your reset password link in http://127.0.0.1:3000/reset-password/' + reset_password_token
        mail.send(msg)
