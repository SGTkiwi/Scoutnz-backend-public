...
class UserMembershipModel(db.Model):
    ...
    user_membership_id = db.Column(db.Integer, primary_key=True, autoincrement=True, nullable=False)
    ...
    # foreign key
    user_profile_id = db.Column(db.Integer, db.ForeignKey("user_profile.user_profile_id"), nullable=False, unique=True)
    
    # relationship
    user_profile = db.relationship("UserProfileModel", back_populates="user_membership")
    user_membership_payment = db.relationship("UserMembershipPaymentModel", back_populates="user_membership", lazy="dynamic", cascade="all, delete")
