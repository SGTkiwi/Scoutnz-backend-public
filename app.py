...
def create_app():
    ...
    app.config[
        "MAIL_USERNAME"
    ] = "hanjun0818@naver.com"  # replace with your naver email id
    app.config[
        "MAIL_PASSWORD"
    ] = "Han@132435"  # replace with your password or app-specific password if applicable
    ...
    def add_claims_to_jwt(identity):
        user = models.AccountModel.query.filter(
            models.AccountModel.account_id == identity
        ).first()
    ...
    api.register_blueprint(routes.JobsBlueprint)
    api.register_blueprint(routes.BookmarkBlueprint)
    api.register_blueprint(routes.FeaturedJobsBlueprint)

    @app.route("/")
    def index():
        return jsonify({"message": "Development server is running"})
