...
class UserPasswordReset(MethodView):
    ...
    def post(self):
        ...
        target_account = AccountModel.query.filter((AccountModel.email == email) & (AccountModel.account_type == account_type)).first()
        
        if not target_account:
            abort(404, message="Email does not exist")
        
        reset_password_token = create_access_token(identity=target_account.account_id, expires_delta=timedelta(minutes=10))
...
class UserPasswordReset(MethodView):
    ...
    def post(self, reset_token):
        try:
            ...
            target_account = AccountModel.query.filter(AccountModel.account_id == account_id).first()
            if not target_account:
                abort(404, message="Invalid token")
            
            target_account.password = bcrypt.generate_password_hash(new_password).decode("utf-8")
