...
def cv():
    user_id = (
        UserProfileModel.query.filter(UserProfileModel.account_id == get_jwt_identity())
        .first()
        .user_profile_id
    )
...
class CreateCV(MethodView):
    ...
    def post(self, cv_data):
        new_cv = CvModel(
            ...
            user_profile_id=UserProfileModel.query.filter(
...
class UpdateCV(MethodView):
    ...
    def put(self, cv_data, cv_id):
        ...
        cv_to_update = CvModel.query.filter(
            (CvModel.cv_id == cv_id) & (CvModel.user_profile_id == user_id)
        ).first()
        if not cv_to_update:
    ...
    def delete(self, cv_id):
        ...
        cv_to_delete = CvModel.query.filter(
            (CvModel.cv_id == cv_id) & (CvModel.user_profile_id == user_id)
        ).first()
    ...
    def get(self, cv_id):
        ...
        cv_return = CvModel.query.filter(
            (CvModel.cv_id == cv_id) & (CvModel.user_profile_id == user_id)
        ).first()
        if not cv_return:
            abort(404, message="CV not found")
