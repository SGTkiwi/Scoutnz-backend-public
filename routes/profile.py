...
blp = Blueprint(
    ...
    description="Operations on user and business profiles",
)
...
class UserProfile(MethodView):
    ...
    def get(self):
        user_profile = UserProfileModel.query.filter(
    ...
    @jwt_required(fresh=True)
    def delete(self):
        user_to_delete = UserProfileModel.query.filter(
            UserProfileModel.account_id == get_jwt_identity()
        ).first()
        if not user_to_delete:
            abort(404, message="User Profile not found")
        account_to_delete = AccountModel.query.filter(
            AccountModel.account_id == get_jwt_identity()
        ).first()
...
class BusinessProfile(MethodView):
    ...
    def get(self):
        business_profile = BusinessProfileModel.query.filter(
    ...
    @jwt_required(fresh=True)
    def delete(self):
        business_to_delete = BusinessProfileModel.query.filter(
            BusinessProfileModel.account_id == get_jwt_identity()
        ).first()
