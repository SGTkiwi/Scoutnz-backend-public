...
blp = Blueprint(
    ...
    description="Application manager for user, apply for job, view applied job",
)
...
class UserApplication(MethodView):
    ...
    def get(self):
        user_id = (
            UserProfileModel.query.filter(
                UserProfileModel.account_id == get_jwt_identity()
            )
            .first()
            .user_profile_id
        )
        cv_ids = [
            cv.cv_id
            for cv in CvModel.query.filter(CvModel.user_profile_id == user_id).all()
        ]
        list_of_application = ApplicationModel.query.filter(
            ApplicationModel.cv_id.in_(cv_ids)
        ).all()
    ...
    def post(self, application_data):
        existing_application = ApplicationModel.query.filter_by(
            cv_id=application_data["cv_id"], job_post_id=application_data["job_post_id"]
        ).first()

        if existing_application:
            return abort(409, message="You have already applied for this job post")
        ...
        new_application.application_handler()
