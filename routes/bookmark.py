...
blp = Blueprint("bookmark", __name__, url_prefix="/api/bookmark", description="Bookmark manager for user, to accept and view applicants")
...
class BookmarkList(MethodView):
    ...
    def get(self):
        try:
            ...
            bookmarks = BookmarkModel.query.filter(BookmarkModel.account_id == account_id).all()
...
class AddBookmark(MethodView):
    ...
    def post(self, job_post_id:int):
        try:
            ...
            if not AccountModel.query.filter(AccountModel.account_id == account_id).first():
                abort(404, message="Account not found")
                
            if not JobPostModel.query.filter(JobPostModel.job_post_id == job_post_id).first():
                abort(404, message="Job post not found")
                
            if BookmarkModel.query.filter(BookmarkModel.job_post_id == job_post_id, BookmarkModel.account_id == account_id).first():
...
class RemoveBookmark(MethodView):
    ...
    def post(self, job_post_id:int):
        try:
            ...
            pre_check = BookmarkModel.query.filter(BookmarkModel.job_post_id == job_post_id, BookmarkModel.account_id == account_id).first()
