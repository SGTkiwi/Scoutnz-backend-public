...
blp = Blueprint("job_post", __name__, url_prefix="/api/job_post", description="Operations on job post")
...
class ViewAllJobPost(MethodView):
    ...
    def get(self):
        ...
        list_of_job_post = JobPostModel.query.filter(JobPostModel.business_profile_id == business_id).all()
...
class CreateJobPost(MethodView):
    ...
    def post(self, job_post_data):
        new_job_post = JobPostModel(
            ...
            business_profile_id = BusinessProfileModel.query.filter(BusinessProfileModel.account_id == get_jwt_identity()).first().business_profile_id
        )
...
class UpdateJobPost(MethodView):
    ...
    def put(self, job_post_data, job_post_id):
        business_id = BusinessProfileModel.query.filter(BusinessProfileModel.account_id == get_jwt_identity()).first().business_profile_id
        job_post_to_update = JobPostModel.query.filter((JobPostModel.job_post_id == job_post_id) &
    ...
    def delete(self, job_post_id):
        business_id = BusinessProfileModel.query.filter(BusinessProfileModel.account_id == get_jwt_identity()).first().business_profile_id
        job_post_to_delete = JobPostModel.query.filter((JobPostModel.job_post_id == job_post_id) &
                                                       (JobPostModel.business_profile_id == business_id)).first()
        ...
        return {"message": "Job Post deleted successfully"}, 204
    
    @jwt_required(fresh=True)
    @blp.response(200, ExtendedJobPostSchema)
    def get(self, job_post_id):
        business_id = BusinessProfileModel.query.filter(BusinessProfileModel.account_id == get_jwt_identity()).first().business_profile_id
        job_post_return = JobPostModel.query.filter((JobPostModel.job_post_id == job_post_id) &
                                                    (JobPostModel.business_profile_id == business_id)).first()
